...
| 个人模型配置 | BYOK：保存、替换、测试和删除自己的 DeepSeek Key；AES-GCM 加密与用户隔离 |
...
管理员先执行 `big-event/src/main/resources/db/knowledge/byok.sql`（已有数据库升级必须手动执行），并一次性生成、保存服务器加密主密钥。生成方法与备份注意事项见 [BYOK 使用与升级说明](docs/byok.md)。

在启动后端的终端中加载已保存的配置：

```powershell
$env:AI_CREDENTIAL_MASTER_KEY=[Environment]::GetEnvironmentVariable('AI_CREDENTIAL_MASTER_KEY', 'User')
$env:DEEPSEEK_CHAT_MODEL='deepseek-v4-flash'
mvn spring-boot:run
```

用户登录后，在 AI 助手右上角「模型配置」保存自己的 DeepSeek API Key，即时生效，无需重启后端。未配置时不会借用公共 Key；模型费用由用户承担。仅支持固定官方地址，测试连接只验证模型列表可访问，不保证余额充足。仓库内的 `.env.example` 不包含真实凭证。
...
| `AI_CREDENTIAL_MASTER_KEY` | 空 | 用户凭据加密主密钥，32 字节 Base64；须安全持久化和备份 |
