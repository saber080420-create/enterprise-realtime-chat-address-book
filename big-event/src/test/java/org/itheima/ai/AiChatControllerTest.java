...
class AiChatControllerTest {
    ...
    @Test void concurrentUsersKeepTheirOwnGatewayAfterLeavingLoginThread() throws Exception {
        var credentials = org.mockito.Mockito.mock(AiCredentialService.class);
        var first = org.mockito.Mockito.mock(DeepSeekGateway.class);
        var second = org.mockito.Mockito.mock(DeepSeekGateway.class);
        org.mockito.Mockito.when(credentials.forUser(1)).thenReturn(first);
        org.mockito.Mockito.when(credentials.forUser(2)).thenReturn(second);
        org.mockito.Mockito.when(first.configured()).thenReturn(true);
        org.mockito.Mockito.when(second.configured()).thenReturn(true);
        var started = new CountDownLatch(2);
        var release = new CountDownLatch(1);
        for (var model : List.of(first, second)) {
            org.mockito.Mockito.doAnswer(call -> {
                assertNull(ThreadLocalUtil.get());
                started.countDown(); release.await(3, TimeUnit.SECONDS);
                return null;
            }).when(model).stream(org.mockito.ArgumentMatchers.anyList(), org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
        }
        controller = new AiChatController(gateway(false));
        org.springframework.test.util.ReflectionTestUtils.setField(controller, "credentials", credentials);
        try {
            ThreadLocalUtil.set(Map.of("id", 1)); controller.chat(request(), new MockHttpServletResponse());
            ThreadLocalUtil.set(Map.of("id", 2)); controller.chat(request(), new MockHttpServletResponse());
            ThreadLocalUtil.remove(); assertTrue(started.await(2, TimeUnit.SECONDS));
            org.mockito.Mockito.verify(first).stream(org.mockito.ArgumentMatchers.anyList(), org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
            org.mockito.Mockito.verify(second).stream(org.mockito.ArgumentMatchers.anyList(), org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
