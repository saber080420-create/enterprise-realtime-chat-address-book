...
public class DeepSeekGateway implements ChatModelGateway {
    ...
    // Immutable request-scoped adapter: never mutate this singleton's credentials.
    DeepSeekGateway withUserKey(String userKey) {
        return new DeepSeekGateway(mapper, userKey, "https://api.deepseek.com", model);
    }

    void testConnection() throws IOException {
        HttpURLConnection connection = (HttpURLConnection) URI.create(baseUrl + "/models").toURL().openConnection();
        connection.setInstanceFollowRedirects(false);
        connection.setConnectTimeout(5000);
        connection.setReadTimeout(10000);
        connection.setRequestProperty("Authorization", "Bearer " + key);
        try {
            if (connection.getResponseCode() != 200) throw new IOException("Credential check failed");
