...
public class AiChatController {
    ...
    public Result<?> status() {
        return Result.success(Map.of("configured", credentials == null ? gateway.configured() : credentials.status(currentUser()).get("configured"), "model", gateway.model(),
    ...
    public ResponseEntity<SseEmitter> chat(@RequestBody AiChatRequest request, HttpServletResponse response) {
        ...
        // Resolve credentials before dispatch: worker threads do not inherit login ThreadLocal.
        final ChatModelGateway requestGateway = credentials == null ? gateway : credentials.forUser(userId);
        if (!requestGateway.configured()) throw new AiRequestException(503, "请先在模型配置中保存自己的 DeepSeek API Key");
            ...
            workers.execute(() -> {
                ...
                try {
                    ...
                    if (request.summaryMode()) {
                        summaries.generate(userId, request, requestGateway, sink, cancellation);
                        ...
                        if (retrieval.sources().isEmpty()) {
                            ...
                            requestGateway.streamGrounded(question, retrieval.sources(), sink, cancellation);
                        }
                    } else requestGateway.stream(request.messages(), sink, cancellation);
                } catch (Exception e) {
